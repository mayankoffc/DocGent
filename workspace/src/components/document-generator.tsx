...
export function DocumentGenerator() {
    ...
    // Local state for the viewer dialog to be independent of the form
    ...
    async function onSubmit(values: FormSchemaType) {
        ...
        try {
            ...
            toast({
                variant: "destructive",
                title: t('error'),
                description: t('toastDocGenError'),
            });
        } finally {
            setIsLoading(false);
        }
    ...
    const handleSaveToGoogleDrive = async () => {
        ...
        setIsUploading(true);
        toast({ title: "Saving to Google Drive...", description: "Your document is being uploaded." });
        ...
        try {
            ...
            const promptStart = form.getValues('prompt').substring(0, 30).replace(/\s/g, '_');
            ...
            toast({
                variant: "destructive",
                title: "Upload Failed",
                description: "Could not save your document to Google Drive. Please try again.",
            });
        } finally {
                                        ...
                                        render={({ field }) => (
                                            ...
                                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                                ...
                                                <SelectContent>
                                                    ...
                                                    <SelectItem value="ultra">{t('docgenQualityUltra')}</SelectItem>
                                                </SelectContent>
                                            </Select>
                                            <FormMessage />
                                            </FormItem>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className={cn(isTimetable && "hidden")}>
                                            <FormLabel className="flex items-center gap-2"><BookOpen /> {isPresentation ? t('docgenSlidesLabel') : t('docgenPagesLabel')}: {field.value}</FormLabel>
                                            <FormControl>
                                                <Slider
